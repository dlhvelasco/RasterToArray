...
ncr = '/home/dwight.velasco/scratch1/THESIS/Grid/NCR/NCR_centroid-SPP.csv'
...
ds = rasterio.open("/home/dwight.velasco/scratch1/THESIS/Renders/old-ordered-tests/NCR_raster.tif", 'r')
clip = ds.read()
print(clip.shape)

preds_means = np.mean(clip, axis=0)
preds_means = np.array(preds_means)

preds_means = list(itertools.chain.from_iterable(preds_means))
preds_means = [x for x in preds_means if x > -1]
print(len(preds_means))
# print(preds_means)
...
for idxday in range(1461):
    clip_nonan = list(itertools.chain.from_iterable(clip[idxday,...]))
    clip_nonan = [x for x in clip_nonan if x > -1]
    # print(len(clip_nonan))
    # print(np.mean(clip_nonan))
    preds.append(np.mean(clip_nonan))
...
df_station = pd.read_csv("/home/dwight.velasco/scratch1/THESIS/Renders/SPP-NCR/NCR1518dailySPP.csv")
print(df_station["all_stations_nonan"].head())
date_df['means'] = df_station["all_stations_nonan"].tolist()
...
output_file = '/home/dwight.velasco/scratch1/THESIS/Renders/SPP-NCR-raster.tif'
